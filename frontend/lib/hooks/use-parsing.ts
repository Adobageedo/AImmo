...
function convertToExtractedLeaseData(parsed?: ParsedLease): ExtractedLeaseData | undefined {
    ...
    return {
        ...
        construction_year: (parsed as any).construction_year,
        last_renovation_year: (parsed as any).last_renovation_year,
        energy_class: (parsed as any).energy_class,
        ges_class: (parsed as any).ges_class,
        purchase_price: (parsed as any).purchase_price,
        purchase_date: (parsed as any).purchase_date,
        current_value: (parsed as any).current_value,
        property_tax: (parsed as any).property_tax,
...
export function useParsing(options: UseParsingOptions = {}) {
    ...
    const convertToParsedLease = (data: ExtractedLeaseData): ParsedLease => {
        return {
            ...
            last_renovation_year: data.last_renovation_year,
            energy_class: data.energy_class,
            ges_class: data.ges_class,
